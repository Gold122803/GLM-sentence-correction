// ==UserScript==
// @name         초월 교정기 for eden-chat v4.3.4
// @namespace    http://tampermonkey.net/
// @version      4.3.4
// @description  eden-chat AI 메시지를 Gemini/DeepSeek/OpenRouter로 자동 교정·교체. v4.3.4: document.body 준비 후 UI를 생성하도록 실행 시점 안정화.
...
(function () {
    ...
    const BOOT_FLAG = '__transCorrectorEdenChatBooted';

    function bootTransCorrector() {
...
- 오직 교정된 전체 본문만 출력한다.`;
    ...
    panel.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
            <h4 style="margin:0;font-size:16px;color:#1A1918;font-family:sans-serif;">초월 교정 설정 v4.3.3</h4>
    ...
    function startTransCorrectorWhenBodyReady() {
        if (window[BOOT_FLAG] || !document.body) return;
        window[BOOT_FLAG] = true;
        bootTransCorrector();
    }

    if (document.body) {
        startTransCorrectorWhenBodyReady();
    } else {
        document.addEventListener('DOMContentLoaded', startTransCorrectorWhenBodyReady, { once: true });
        window.addEventListener('load', startTransCorrectorWhenBodyReady, { once: true });
        setTimeout(startTransCorrectorWhenBodyReady, 1500);
    }

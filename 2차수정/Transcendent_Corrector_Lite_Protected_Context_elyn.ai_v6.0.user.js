// ==UserScript==
// @name         초월 교정기 Lite Protected Context for elyn.ai
// @namespace    http://tampermonkey.net/
// @version      6.0.0-protected-context
// @description  elyn.ai AI 메시지를 교정·교체. 코드블럭을 보호 토큰으로 보존하고 맥락 원문으로 참조.
// @match        https://elyn.ai/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_addStyle
...
- 오직 교정된 전체 본문만 출력한다.`;
    ...
    panel.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
            <h4 style="margin:0;font-size:16px;color:#1A1918;font-family:sans-serif;">초월 교정 설정 v6.0</h4>
    ...
    function getSavedApiKey(provider) {
        ...
        if (provider === 'vertex') return GM_getValue('vertexApiKey', '');
    ...
    function saveProviderFields(provider) {
        ...
        if (provider === 'deepseek') {
            ...
            GM_setValue('vertexApiKey', apiKeyInput.value.trim());
    ...
    function getVertexEndpoint(modelOrEndpoint) {
    ...
    function callVertex(text, overrideModel = null, userContext = '') {
        return new Promise((resolve, reject) => {
            ...
            if (!apiKey) { reject(new Error('Vertex API key/access token is not set.')); return; }
            const modelOrEndpoint = overrideModel || GM_getValue('vertexModel', DEFAULT_VERTEX_MODEL);
            const { contextBlock, protectedBlocks } = buildCorrectionInput(text, userContext);
            let endpoint;
            ...
            GM_xmlhttpRequest({
                ...
                onload(res) {
                    try {
                        const data = JSON.parse(res.responseText || '{}');
                        if (res.status < 200 || res.status >= 300) { reject(new Error(data?.error?.message || `Vertex API error ${res.status}`)); return; }
                        if (data.error) { reject(new Error(data.error.message)); return; }
                        const candidate = data.candidates?.[0];
                        const raw = getGeminiTextFromCandidate(candidate);
                        if (!raw) { reject(new Error(`Vertex response body is empty. finishReason=${candidate?.finishReason || 'unknown'}.`)); return; }
                        resolve(restoreProtectedBlocks(stripOuterFence(raw), protectedBlocks));
                    } catch (e) { reject(e); }
                },
                ontimeout() { reject(new Error('Vertex request timed out.')); },
                onerror() { reject(new Error('Vertex network error occurred.')); },
            });
        });
    }

// ==UserScript==
// @name         초월 교정기 Inline for babechat.ai
// @namespace    http://tampermonkey.net/
// @version      5.1.2
// @updateURL    https://raw.githubusercontent.com/Gold122803/GLM-sentence-correction/main/release/babechat-inline.user.js
...
- 오직 교정된 전체 본문만 출력한다.`;
    ...
    function formatProtectedCodeContext(item) {
        return `${item.token}\n${item.block
            .replace(/^```/, FENCE_OPEN_SUB)
            .replace(/```\s*$/, FENCE_CLOSE_SUB)}`;
    }
    function createProtectedCorrectionInput(text) {
        ...
        const protect = (block, kind) => {
            ...
            const item = { token, block, kind };
            blocks.push(item);
            if (kind === 'code') codeContexts.push(formatProtectedCodeContext(item));
            return token;
        };
    ...
    function buildCorrectionInput(text, userContext = '') {
        ...
        const codeContext = protection.codeContexts.length
            ? `[보존 코드블럭 맥락 - 참고용, 교정/출력 대상 아님]\n아래 내용은 ${FENCE_OPEN_SUB} / ${FENCE_CLOSE_SUB}로 원래 코드블럭 경계를 표시한 참고용 원문이다. 교정 결과에는 토큰만 유지하고, 이 맥락 원문을 직접 출력하거나 수정하지 않는다.\n${protection.codeContexts.join('\n\n')}\n\n`
            : '';

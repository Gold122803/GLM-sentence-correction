// ==UserScript==
// @name         초월 교정기 for elyn.ai v3
// @namespace    http://tampermonkey.net/
// @version      3.0
// @description  elyn.ai AI 메시지를 Gemini로 자동 교정·교체. 팝업 미리보기 및 리롤 지원.
// @match        https://elyn.ai/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_addStyle
...
- 교정 외의 부연 설명, 인사말, 감상, 주석 등은 절대 출력하지 마십시오. 오직 교정된 본문만 제공하십시오`;
    ...
    const clampButtonPosition = () => {
        ...
        if (isNaN(l) || l < 0) l = 20; if (l > maxX) l = maxX - 20;
        if (isNaN(t) || t < 0) t = 20; if (t > maxY) t = maxY - 20;
        settingBtn.style.left = l + 'px'; settingBtn.style.top = t + 'px';
    ...
    function findLastPencilBtn() {
        ...
        const pencilBtns = Array.from(allBtns).filter(btn => btn.querySelector('.lucide-pencil'));
        return pencilBtns[pencilBtns.length - 1] || null;
    }

    function findEditArea() {
        return document.querySelector('div[contenteditable="true"].whitespace-pre-wrap');
    }

    function findSaveBtn() {
        return Array.from(document.querySelectorAll('button')).find(
            btn => btn.className.includes('hover:bg-green-500')
        ) || null;
    }
    ...
    async function autoCorrect() {
        ...
        try {
            ...
            if (usePreview) {
                ...
                const corrected = await callGemini(original);
